...
begin
  if Assigned(CurrentWebSession) then  
    FJSName := 'O' + IdentDelim + TExtThread(CurrentWebSession).GetSequence + IdentDelim else
    FJSName := 'O' + IdentDelim + 'mainthread' + IdentDelim;
end;
...
constructor TExtObject.CreateSingleton(Attribute: string = '');
begin
  if Attribute = '' then
    FJSName := JSClassName
  else
...
function TExtObject.CharsToPixels(Chars: Integer): Integer;
begin
...
function TExtObject.LinesToPixels(Lines: Integer): Integer;
begin
...
procedure TExtObject.CreateVar(JS: string);
begin
  CreateJSName;
  if Assigned(CurrentWebSession) then  
...
procedure TExtObject.CreateVarAlt(JS: string);
begin
  CreateJSName;
  TExtThread(CurrentWebSession).GarbageAdd(JSName, Self);
  insert('/*' + JSName + '*/', JS, length(JS) - IfThen(pos('});', JS) <> 0, 2, 1));
...
procedure TExtObject.DeleteFromGarbage;
begin
  if CurrentWebSession <> nil then
    TExtThread(CurrentWebSession).GarbageDelete(Self);
end;
...
begin
  if CurrentWebSession <> nil then
    TExtThread(CurrentWebSession).GarbageRemove(Self);
  inherited;
end;
...
constructor TExtObject.Create(Owner: TExtObject = nil);
begin
  if Owner = nil then
...
constructor TExtObject.CreateInternal(Owner: TExtObject; Attribute: string);
begin
...
begin
  ...
  else if pos(IdentDelim + '.', Value) = 0 then
    FJSCommand := Value
  else
...
var
  lJSName, JSC: string;
begin
  if Assigned(CurrentWebSession) and (JS <> '') then
  begin
    if (pos('.nm="', JS) = 0) and (JS[length(JS)] = ';') and not(pos(DeclareJS, JS) in [1, 2]) then
    begin
      if (JSCommand <> '') and (pJSName <> '') and not IsParent(pJSName) then
      begin
        JSC := JSCommand;
        JSCommand := '';
      ...
      if not(pos(IdentDelim + '.get', FJSCommand) in [4 .. 9]) then
        FJSCommand := '';
      JSCommand := JS;
    end;
    // else JSCommand := '';
...
constructor TExtObject.AddTo(List: TExtObjectList);
begin
  if JSName = '' then
  begin
    CreateVar(JSClassName + '({});');
...
constructor TExtObject.Init(Method: TExtFunction);
begin
  CreateJSName;
...
constructor TExtObject.Init(Command: string);
begin
  CreateJSName;
  TExtThread(CurrentWebSession).GarbageAdd(JSName, Self);
  Created := true;
...
function TExtObject.JSArray(JSON: string; SquareBracket: boolean = true): TExtObjectList;
begin
...
*)
function TExtObject.JSObject(JSON: string; ObjectConstructor: string = ''; CurlyBracket: boolean = true): TExtObject;
begin
...
procedure TExtObject.HandleEvent(const AEvtName: string);
begin
end;
...
function TExtObject.JSFunction(Params, Body: string): TExtFunction;
begin
...
function TExtObject.JSFunction(Body: string): TExtFunction;
begin
...
procedure TExtObject.JSFunction(Name, Params, Body: string);
begin
...
function TExtObject.Ajax(Method: TExtProcedure): TExtFunction;
begin
  Result := Ajax(Method, []);
end;
...
begin
  Result := FindMethod(Method, MetName, ObjName);
  JSCode('Ext.Ajax.request({url:"' + MethodURI(MetName) + '",params:"Ajax=1&"+' + Params + IfThen(ObjName = '', '',
    '+"&Obj=' + ObjName + '"') + ',success:AjaxSuccess,failure:AjaxFailure});');
...
begin
  ...
  begin
    if Forms[I] is TExtFormField then
    begin
    ...
    if I <> high(Forms) then
      S := S + '+"&"+';
  end;
  Result := Ajax(Method, S);
end;

function TExtObject.AjaxSelection(Method: TExtProcedure; SelectionModel: TExtObject; Attribute, TargetQuery: string;
  Params: array of const): TExtFunction;
var
  CurrentResponse: string;
  MetName, ObjName: string;
begin
  ...
  begin
    CurrentResponse := Response;
    Response := '';
    with TExtSelectionRowModel(SelectionModel) do
    begin
      JSCode('var Sel=[];');
      TExtDataStore(TExtSelectionRowModel(SelectionModel).GetStore)
    ...
    with Params[I] do
      if Odd(I) then
        case VType of
          ...
          vtString:
            Result := Result + SurroundAjaxParam(VString^);
          vtWideString:
...
{$ENDIF}
          vtObject:
            Result := Result + '"+' + TExtObject(VObject).ExtractJSCommand + '+"';
          vtInteger:
...
procedure TExtObject.AjaxCode(MethodName: string; RawParams: string; Params: array of const);
begin
  ...
  begin
    with A[I] do
      case VType of
        vtObject:
          begin
            ...
            begin
              ...
              if (Command <> '') and A[I + 1].VBoolean then
              begin
              ...
              begin
                ...
                if InJSFunction and (pos(JSName, Command) = 1) then
                begin
        ...
        vtAnsiString:
          Result := Result + StrToJS(string(VAnsiString));
        vtString:
          Result := Result + StrToJS(VString^);
        vtWideString:
          Result := Result + StrToJS(WideString(VWideString));
{$IFDEF UNICODE}
        vtUnicodeString:
          Result := Result + StrToJS(string(VUnicodeString));
{$ENDIF}
        vtInteger:
          Result := Result + IntToStr(VInteger);
        vtBoolean:
          Result := Result + IfThen(VBoolean, 'true', 'false');
        vtExtended:
          Result := Result + AnsiReplaceStr(FloatToStr(VExtended^), ',', '.');
        vtCurrency:
          Result := Result + CurrToStr(VCurrency^);
        vtInt64:
          Result := Result + IntToStr(VInt64^);
...
function TExtObject.VarToJSON(Exts: TExtObjectList): string;
begin
...
function TExtObject.ArrayToJSON(Strs:
...
function TExtObject.ArrayToJSON(Ints:
