...
{$ENDIF}
    ...
    constructor Create(pTitle: string; ASessionClass: TCustomWebSessionClass; pPort: word = 2014;
...
var
  ...
  threadvar CurrentWebSession: TFCGISession; // current FastCGI session object
...
function CreateWebApplication(const ATitle: string; ASessionClass: TCustomWebSessionClass; APort: word = 2014;
  AMaxIdleMinutes: word = 30; AShutdownAfterLastThreadDown: Boolean = false; AMaxConns: Integer = 1000)
  : TFCGIApplication;
...
function CurrentFCGIThread: TFCGISession;
begin
...
function CreateWebApplication(const ATitle: string; ASessionClass: TCustomWebSessionClass; APort: word = 2014;
  ...
  : TFCGIApplication;
begin
  Result := TFCGIApplication.Create(ATitle, ASessionClass, APort, AMaxIdleMinutes, AShutdownAfterLastThreadDown,
    AMaxConns);
end;

type
  // FastCGI record types, i.e. the general function that the record performs
  TRecType = (rtBeginRequest = 1, rtAbortRequest, rtEndRequest, rtParams, rtStdIn, rtStdOut, rtStdErr, rtData,
    rtGetValues, rtGetValuesResult, rtUnknown);
  ...
  private
    ...
    FGarbage, FKeepConn: Boolean; // Not used
    FRequest: string;
  ...
  public
    ...
    // If false generates 'cache-control:no-cache' and 'pragma:no-cache' in HTTP header, default is false
...
threadvar _CurrentFCGIThread: TFCGIThread;
// Current FastCGI thread address assigned by <link TFCGIThread.SetCurrentFCGIThread, SetCurrentFCGIThread> method
...
begin
  if Application.FThreadsCount < 0 then
    Application.FThreadsCount := 0;
...
procedure TFCGIThread.SetResponseHeader(Header: AnsiString);
begin
...
var
  ...
  I: Integer;
begin
  if pRecType = rtStdOut then
  begin
    if FRequestMethod = rmHead then
      S := '';
    FSession.CustomResponseHeaders['content-type'] := FSession.ContentType;
    if not BrowserCache and not FSession.IsDownload then
    begin
...
begin
  ...
  begin
    case Status of
      psCantMPXConn:
        FSession.Alert('Multiplexing is not allowed.');
      psOverloaded:
        FSession.Alert('Maximum connection limit is ' + IntToStr(Application.MaxConns) + ' and was reached.');
      psUnknownRole:
        FSession.Alert('Unknown FastCGI Role received.');
      psBusy:
        ; // Alert('Session is busy, try later.');
    end;
...
begin
  ...
  if BeginRequest.Role in [rResponder .. rFilter] then
  begin
...
procedure TFCGIThread.ReadRequestHeader(var RequestHeader: TStringList; Stream: AnsiString;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Len[I] > 127 then
      begin
        Len[I] := ((byte(Stream[Pos]) and $7F) shl 24) + (byte(Stream[Pos + 1]) shl 16) + (byte(Stream[Pos + 2]) shl 8)
          + byte(Stream[Pos + 3]);
        inc(Pos, 4);
      end
      else
    ...
    inc(Pos, Len[1]);
    if Param[0] = 'HTTP_COOKIE' then
    begin
      if ParseCookies then
        FSession.FCookies.DelimitedText := URLDecode(Param[1])
    end
    else
...
begin
  ...
  if Pos('multipart/form-data', CT) <> 0 then
    FSession.UploadPrepare(CT, Buffer, I);
...
begin
  ...
  GetValuesResult := '';
  if Values.IndexOf('FCGI_MAX_CONNS') <> -1 then
    AddParam(GetValuesResult, ['FCGI_MAX_CONNS', IntToStr(Application.MaxConns)]);
  if Values.IndexOf('FCGI_MAX_REQS') <> -1 then
    AddParam(GetValuesResult, ['FCGI_MAX_REQS', IntToStr(Application.MaxConns)]);
  if Values.IndexOf('FCGI_MPXS_CONNS') <> -1 then
    AddParam(GetValuesResult, ['FCGI_MPXS_CONNS', '0']);
  Values.Free;
...
begin
  ...
  try
    ...
    begin
      ...
      AccessThread.Enter;
      if Application.ReachedMaxConns then
      begin
      ...
      begin
        ...
        if FScriptName[length(FScriptName)] <> '/' then
            ...
            begin
              ...
              begin
                ...
                case FCGIHeader.RecType of
                  rtBeginRequest:
                    ReadBeginRequest(FCGIHeader, Content);
                  rtAbortRequest:
                    FSession.Logout;
                  rtGetValues:
                    GetValues(Content);
                  rtParams, rtStdIn, rtData:
                    ...
                    else if FSession.IsUpload then
...
begin
  ...
  if FSession.IsDownload or (FSession.IsUpload and (FSession.Browser = brIE)) then
    ...
    Result := {$IFDEF MSWINDOWS}AnsiToUTF8{$ENDIF}(FSession.Response);
end;
...
begin
  ...
  begin
    ...
    if (Now - Thread.LastAccess) > MaxIdleTime then
    begin
      AccessThreads.Enter;
      Thread.Free;
      Threads.Delete(I);
...
{$ENDIF}
  if HasConfig then
    if not Config.ReadBool('FCGI', 'Enabled', true) then
    begin
  ...
  begin
    ...
    if Error = 0 then
      repeat
        ...
        if ((I mod 40) = 0) or GarbageNow then
        begin // A garbage for each 10 seconds
          GarbageThreads;
          GarbageNow := false;
          I := 0;
        end;
...
procedure TFCGISession.SendResponse(const Msg: string);
begin
