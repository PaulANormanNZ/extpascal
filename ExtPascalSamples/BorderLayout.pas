...
begin
  ...
  begin
    ...
    SetStyle('html,body{font:normal 12px verdana;margin:0;padding:0;border:0 none;overflow:hidden;height:100%}' +
      'p{margin:5px}' + '.settings{background:url(' + ExtPath + '/examples/shared/icons/fam/folder_wrench.png)}' +
  ...
  Tree.RootInterface.AppendChild(Node);
  ...
  begin
    ...
    MaxSize := 400;
    Collapsible := True;
    with TExtTabPanel.AddTo(Items) do
    begin
      Border := 0;
      ...
      with TExtGridPropertyGrid.AddTo(Items) do
      begin
        Title := 'Property Grid';
        Closable := True;
...
procedure TBorderLayout.SelectNodeEventBrowserSide;
begin
  ExtMessageBox.Alert('Browser Side', '%0.text')
end;

procedure TBorderLayout.SelectNodeEventServerSide;
begin
